"""
LoRA Wildcard Generator V2 Node for ComfyUI
...
class LoraWildcardGeneratorV2:
    ...
    def generate_lora_entry(self, lora_name, trained_words):
        """
        Generate LoRA syntax entry with V2 format
        ...
        Returns:
            String with LoRA syntax (V2 format)
        """
    ...
    def create_yaml_content(self, entries, wildcard_name):
        """
        Create YAML content from entries (V2: no combination entry)
        ...
        # V2: No 'all-<wildcard_name>' combination entry
        nested_dict = {}
        ...
        # V2: Only individual entries, no combination
        final_dict = {
    ...
    def generate_wildcard(self, json_folder, wildcard_name, output_folder, include_subfolders):
        ...
        try:
            ...
            print(f"\n{'='*60}")
            print(f"Processing JSON files for LoRA wildcards (V2)...")
            ...
            # Create YAML content (V2: no combination)
            print(f"\nGenerating YAML wildcard file (V2 format)...")
            ...
            if entries:
                print("Preview (first 3 entries in V2 format):")
                for i, entry in enumerate(entries[:3]):
                    lora_entry = self.generate_lora_entry(entry['lora_name'], entry['trained_words'])
                    print(f"  {i+1}. {entry['lora_name']}: {lora_entry}")
                print()

            status = f"Successfully generated {yaml_filename} with {len(entries)} entries (V2 format)"
...
NODE_CLASS_MAPPINGS = {
    "LoraWildcardGeneratorV2": LoraWildcardGeneratorV2,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "LoraWildcardGeneratorV2": "LoRA Wildcard Generator V2",
}
